...
function renderTemplate(input, vars) {
  ...
  return input.replace(/\{\{\s*([\w\.]+)\s*\}\}/g, (_, key) => {
    const value = key.split(".").reduce((acc, part) => acc?.[part], vars);
...
function ses() {
  if (_ses) return _ses;
  _ses = new import_client_ses.SESClient({ region: process.env.AWS_REGION ?? "us-east-1" });
  return _ses;
}
async function sendEmail(args) {
  ...
  if (!subject) return { ok: false, errorCode: "INVALID_TEMPLATE", errorMessage: "Email template requires subject" };
  if (!htmlBody && !textBody) return { ok: false, errorCode: "INVALID_TEMPLATE", errorMessage: "Email template requires html_body or text_body" };
  try {
    const result = await ses().send(new import_client_ses.SendEmailCommand({
      Source: from,
      Destination: { ToAddresses: [args.to] },
      ReplyToAddresses: replyTo ? [replyTo] : void 0,
...
async function sendWhatsApp(args) {
  ...
  if (!args.template.wa_template_name) {
    return { ok: false, errorCode: "INVALID_TEMPLATE", errorMessage: "WhatsApp template requires wa_template_name (approved by Meta)" };
  }
  const to = args.to.replace(/^\+/, "");
  const components = args.template.wa_components ? injectPayload(args.template.wa_components, args.payload) : buildSimpleBody(args.payload);
  const body = {
  ...
  try {
    ...
    return {
      ok: false,
      errorCode: "WA_NETWORK",
      errorMessage: err.message ?? String(err)
    };
  }
}
function buildSimpleBody(payload) {
  const parameters = Object.values(payload).map((v) => ({
    type: "text",
    text: String(v ?? "")
  }));
...
function injectPayload(components, payload) {
  return JSON.parse(
    JSON.stringify(components).replace(/\{\{\s*([\w\.]+)\s*\}\}/g, (_, key) => {
      const value = key.split(".").reduce((acc, p) => acc?.[p], payload);
      return value !== void 0 && value !== null ? String(value).replace(/"/g, '\\"') : "";
    })
  );
}

// src/sender.ts
var BATCH_SIZE = parseInt(process.env.BATCH_SIZE ?? "50");
var MAX_ATTEMPTS = parseInt(process.env.MAX_ATTEMPTS ?? "3");
async function handler(_event) {
  const sb = supabase();
  const startedAt = Date.now();
  const { data: pending, error: fetchErr } = await sb.from("message_queue").select("id, workspace_id, lead_id, template_key, channel, to_address, payload, attempts").eq("status", "pending").lte("scheduled_at", (/* @__PURE__ */ new Date()).toISOString()).order("scheduled_at").limit(BATCH_SIZE);
  if (fetchErr) throw new Error(`Fetch queue failed: ${fetchErr.message}`);
  if (!pending || pending.length === 0) {
    console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] sender: nothing to send`);
    return { processed: 0, ok: 0, failed: 0 };
  }
  console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] sender: ${pending.length} mensajes a procesar`);
  const templateKeys = Array.from(new Set(pending.map((p) => p.template_key)));
  const { data: templates } = await sb.from("message_templates").select("template_key, channel, subject, html_body, text_body, wa_template_name, wa_language, wa_components, variables").in("template_key", templateKeys);
  ...
  for (const msg of pending) {
    ...
    const attemptStart = /* @__PURE__ */ new Date();
    const result = msg.channel === "email" ? await sendEmail({ to: msg.to_address, template: tpl, payload: msg.payload ?? {} }) : await sendWhatsApp({ to: msg.to_address, template: tpl, payload: msg.payload ?? {} });
    ...
    if (result.ok) {
      ...
      await sb.from("leads_master").update({
        last_contacted_at: (/* @__PURE__ */ new Date()).toISOString(),
      ...
      await sb.from("message_queue").update({
        ...
        scheduled_at: exhausted ? null : new Date(Date.now() + 6e4 * attemptNumber * 5).toISOString()
      }).eq("id", msg.id);
      failed++;
    }
  }
  console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] sender: done in ${Date.now() - startedAt}ms \u2014 ok=${ok} failed=${failed}`);
  return { processed: pending.length, ok, failed };
}
async function markFailed(msg, code, message) {
  const sb = supabase();
  await sb.from("message_queue").update({
    status: "failed",
