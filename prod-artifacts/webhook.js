...
async function handler(event) {
  ...
  try {
    if (path.startsWith("/webhook/ses")) {
      return await handleSes(event);
    }
    if (path.startsWith("/webhook/whatsapp")) {
      return await handleWhatsApp(event);
    }
    if (path.startsWith("/webhook/external")) {
      return await handleExternalEvent(event);
    }
...
async function handleSes(event) {
  ...
  const sb = supabase();
  const { data: queue } = await sb.from("message_queue").select("id, workspace_id, lead_id").eq("ses_message_id", sesMessageId).maybeSingle();
  if (!queue) {
...
async function handleWhatsApp(event) {
  ...
  if (signature && appSecret) {
    const expected = "sha256=" + import_crypto.default.createHmac("sha256", appSecret).update(event.body ?? "").digest("hex");
  ...
  for (const entry of body.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      for (const s of value.statuses ?? []) {
        const { data: queue } = await sb.from("message_queue").select("id, workspace_id, lead_id").eq("wa_message_id", s.id).maybeSingle();
        if (!queue) continue;
        await sb.from("lead_events").insert({
          workspace_id: queue.workspace_id,
          lead_id: queue.lead_id,
        ...
        if (s.status === "failed") {
          await sb.from("message_queue").update({ status: "failed", last_error: JSON.stringify(s.errors) }).eq("id", queue.id);
        }
      }
      for (const m of value.messages ?? []) {
        const fromPhone = m.from;
        const { data: lead } = await sb.from("leads_master").select("id, workspace_id").or(`phone.eq.${fromPhone},phone.eq.+${fromPhone}`).maybeSingle();
        if (lead) {
          await sb.from("lead_events").insert({
...
async function handleExternalEvent(event) {
  ...
  const sb = supabase();
  const { data: endpoint } = await sb.from("webhook_endpoints").select("id, workspace_id, secret, active").eq("url_token", token).maybeSingle();
  if (!endpoint || !endpoint.active) return { statusCode: 404, body: "unknown endpoint" };
  const signature = event.headers?.["x-signature"];
  if (signature) {
    const expected = import_crypto.default.createHmac("sha256", endpoint.secret).update(event.body ?? "").digest("hex");
    if (signature !== expected) return { statusCode: 401, body: "invalid signature" };
  }
  const body = JSON.parse(event.body ?? "{}");
  ...
  await sb.from("webhook_endpoints").update({ last_used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", endpoint.id);
